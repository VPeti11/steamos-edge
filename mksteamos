...
_run_mksquashfs() {
    local mksquashfs_options=() image_path="${isofs_dir}/${install_dir}/${arch}/strootfs.sfs"
    rm -f -- "${image_path}"
    [[ ! "${quiet}" == "y" ]] || mksquashfs_options+=('-no-progress' '-quiet')
    mksquashfs "$@" "${image_path}" -noappend "${strootfs_image_tool_options[@]}" "${mksquashfs_options[@]}"
}
...
_mkstrootfs_erofs() {
    ...
    install -d -m 0755 -- "${isofs_dir}/${install_dir}/${arch}"
    local image_path="${isofs_dir}/${install_dir}/${arch}/strootfs.erofs"
    rm -f -- "${image_path}"
    [[ ! "${quiet}" == "y" ]] || mkfs_erofs_options+=('--quiet')
    mkfs_erofs_options+=('-U' '00000000-0000-0000-0000-000000000000' "${strootfs_image_tool_options[@]}")
...
_mkchecksum() {
    ...
    if [[ -e "${isofs_dir}/${install_dir}/${arch}/strootfs.sfs" ]]; then
        sha512sum strootfs.sfs >strootfs.sha512
    elif [[ -e "${isofs_dir}/${install_dir}/${arch}/strootfs.erofs" ]]; then
        sha512sum strootfs.erofs >strootfs.sha512
    fi
    cd -- "${OLDPWD}"
    _msg_info "Done!"
}
...
_mk_pgp_signature() {
    local gpg_options=()
    local strootfs_image_filename="${1}"
    _msg_info "Signing rootfs image using GPG..."

    rm -f -- "${strootfs_image_filename}.sig"
    ...
    gpg --batch --no-armor --no-include-key-block --output "${strootfs_image_filename}.sig" --detach-sign \
        --default-key "${gpg_key}" "${gpg_options[@]}" "${strootfs_image_filename}"
    _msg_info "Done!"
}
...
_make_pacman_conf() {
    ...
    # HookDir is *always* set to the strootfs' override directory
...
_make_custom_strootfs() {
    local passwd=()
    local filename permissions

    install -d -m 0755 -o 0 -g 0 -- "${pacstrap_dir}"

    if [[ -d "${profile}/strootfs" ]]; then
        _msg_info "Copying custom strootfs files..."
        cp -af --no-preserve=ownership,mode -- "${profile}/strootfs/." "${pacstrap_dir}"
...
_make_packages() {
    ...
    if [[ "${quiet}" = "y" ]]; then
        env -u TMPDIR pacstrap -C "${work_dir}/${buildmode}.pacman.conf" -c -G -M -- "${pacstrap_dir}" "${buildmode_pkg_list[@]}" --overwrite --skipinteg '*' &>/dev/null
    else
        env -u TMPDIR pacstrap -C "${work_dir}/${buildmode}.pacman.conf" -c -G -M -- "${pacstrap_dir}" "${buildmode_pkg_list[@]}" --overwrite '*'
    fi
...
_make_customize_strootfs() {
    local passwd=()

    if [[ -e "${profile}/strootfs/etc/passwd" ]]; then
        ...
        done <"${profile}/strootfs/etc/passwd"
        _msg_info "Done!"
    fi

    if [[ -e "${pacstrap_dir}/root/customize_strootfs.sh" ]]; then
        _msg_info "Running customize_strootfs.sh in '${pacstrap_dir}' chroot..."
        _msg_warning "customize_strootfs.sh is deprecated! Support for it will be removed in a future archiso version."
        chmod -f -- +x "${pacstrap_dir}/root/customize_strootfs.sh"
        # Unset TMPDIR to work around https://bugs.archlinux.org/task/70580
        eval -- env -u TMPDIR arch-chroot "${pacstrap_dir}" "/root/customize_strootfs.sh"
        rm -- "${pacstrap_dir}/root/customize_strootfs.sh"
        _msg_info "Done! customize_strootfs.sh run successfully."
    fi
}
...
# Build strootfs filesystem image
_prepare_strootfs_image() {
    _run_once "_mkstrootfs_${strootfs_image_type}"
    _mkchecksum

    if [[ -e "${isofs_dir}/${install_dir}/${arch}/strootfs.sfs" ]]; then
        strootfs_image_filename="${isofs_dir}/${install_dir}/${arch}/strootfs.sfs"
    elif [[ -e "${isofs_dir}/${install_dir}/${arch}/strootfs.erofs" ]]; then
        strootfs_image_filename="${isofs_dir}/${install_dir}/${arch}/strootfs.erofs"
    fi

    if [[ -n "${gpg_key}" ]]; then
        _mk_pgp_signature "${strootfs_image_filename}"
    fi
    if [[ -v cert_list ]]; then
        _cms_sign_artifact "${strootfs_image_filename}"
    fi
}
...
_validate_requirements_strootfs_image_type_squashfs() {
    if ! command -v mksquashfs &>/dev/null; then
        (( validation_error=validation_error+1 ))
        _msg_error "Validating '${strootfs_image_type}': mksquashfs is not available on this host. Install 'squashfs-tools'!" 0
    fi
}

_validate_requirements_strootfs_image_type_ext4+squashfs() {
    if ! { command -v mkfs.ext4 &>/dev/null && command -v tune2fs &>/dev/null; }; then
        (( validation_error=validation_error+1 ))
        _msg_error "Validating '${strootfs_image_type}': mkfs.ext4 and/or tune2fs is not available on this host. Install 'e2fsprogs'!" 0
    fi
    _validate_requirements_strootfs_image_type_squashfs
}

_validate_requirements_strootfs_image_type_erofs() {
    if ! command -v mkfs.erofs &>/dev/null; then
        (( validation_error=validation_error+1 ))
        _msg_error "Validating '${strootfs_image_type}': mkfs.erofs is not available on this host. Install 'erofs-utils'!" 0
    fi
}
...
_validate_common_requirements_buildmode_iso_netboot() {
    ...
    # Check if the specified strootfs_image_type is supported
    if typeset -f "_mkstrootfs_${strootfs_image_type}" &>/dev/null; then
        if typeset -f "_validate_requirements_strootfs_image_type_${strootfs_image_type}" &>/dev/null; then
            "_validate_requirements_strootfs_image_type_${strootfs_image_type}"
        else
            _msg_warning "Function '_validate_requirements_strootfs_image_type_${strootfs_image_type}' does not exist. Validating the requirements of '${strootfs_image_type}' strootfs image type will not be possible."
        fi
    else
        (( validation_error=validation_error+1 ))
        _msg_error "Unsupported image type: '${strootfs_image_type}'" 0
    fi
}
...
_set_overrides() {
    ...
    [[ -n "$strootfs_image_type" ]] || strootfs_image_type="squashfs"
...
_build_iso_base() {
    ...
    pacstrap_dir="${work_dir}/${arch}/strootfs"
    ...
    _run_once _make_custom_strootfs
    _run_once _make_packages
    _run_once _make_version
    _run_once _make_customize_strootfs
    ...
    _run_once _prepare_strootfs_image
}
...
_build_buildmode_iso() {
    local image_name="${iso_name}-${iso_version}-${arch}.img" # Restoring - GuestSneezeOSDev
    local run_once_mode="${buildmode}"
